...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2' # disable info/warning messages
...
# Called by convert_raw_to_numpy
...
def doit(mode, target_directory):
    """Runs convert_raw_to_numpy on every file, and output the data"""
    file_names = _get_dataset_files(dataset_info, mode, '.')

    tot = 0
    for file in file_names:
        engine = tf.python_io.tf_record_iterator(file)
        for i, raw_data in enumerate(engine):
            path = os.path.join(target_directory, f'{tot+i}.pt.gz')
    ...
    print(f' [-] {tot} scenes in the {mode} dataset')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print(' [!] No dataset! Please run me like this: \n\t python tf2torch.py shepard_metzler_5_parts')
        exit()
    ...
    ensure(torch_dataset_path)
    ensure(torch_dataset_path_train)
    ensure(torch_dataset_path_test)

    doit('train', torch_dataset_path_train)
    doit('test', torch_dataset_path_test)
